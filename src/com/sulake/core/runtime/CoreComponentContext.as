package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1399:String = "asset-libraries";
      
      private static const const_1401:String = "library";
      
      private static const const_1402:String = "service-libraries";
      
      private static const const_1407:String = "library";
      
      private static const const_1400:String = "component-libraries";
      
      private static const const_1403:String = "library";
      
      private static const const_1404:String = "error_data";
      
      private static const const_1405:String = "error_cat";
      
      private static const const_1406:String = "error_desc";
       
      
      private var var_364:LibraryLoaderQueue;
      
      private var var_792:IEventDispatcher;
      
      private var var_533:uint;
      
      private var var_365:Function;
      
      private var var_208:Profiler;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         {
            this.var_260.push(new Array());
            this.var_647.push(0);
            _loc3_++;
         }
         this._lastUpdateTimeMs = getTimer();
         attachComponent(this,[new IIDCore()]);
         var_400.addEventListener(Event.ENTER_FRAME,this.onEnterFrame);
         switch(param2 & 0)
         {
            case Core.const_1119:
               debug("Core; using simple frame update handler");
               this.var_365 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_751:
               debug("Core; using complex frame update handler");
               this.var_365 = this.complexFrameUpdateHandler;
               break;
            case Core.const_777:
               debug("Core; using profiler frame update handler");
               this.var_365 = this.profilerFrameUpdateHandler;
               this.var_208 = new Profiler(this);
               attachComponent(this.var_208,[new IIDProfiler()]);
               var_400.addChild(new ProfilerViewer(this.var_208));
               break;
            case Core.const_514:
               debug("Core; using experimental frame update handler");
               this.var_365 = this.experimentalFrameUpdateHandler;
               break;
            case Core.CORE_SETUP_DEBUG:
               debug("Core; using debug frame update handler");
               this.var_365 = this.debugFrameUpdateHandler;
         }
      }
      
      public function getNumberOfFilesPending() : uint
      {
         return this.var_364.length;
      }
      
      public function getNumberOfFilesLoaded() : uint
      {
         return this.var_533 - this.getNumberOfFilesPending();
      }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            try
            {
               ...
               {
                  receivers = this.var_260[i] as Array;
            ...
            if(var_400)
            {
               var_400.removeEventListener(Event.ENTER_FRAME,this.onEnterFrame);
               var_400 = null;
            }
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1404,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1405,String(param3));
         ErrorReportStorage.setParameter(const_1406,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_792 = loadingEventDelegate;
         if(this.var_364 == null)
         {
            this.var_364 = new LibraryLoaderQueue(_debug);
         }
         try
         {
            node = config.child(const_1399)[0];
            if(node != null)
            {
               list = node.child(const_1401);
         ...
         try
         {
            node = config.child(const_1402)[0];
            if(node != null)
            {
               list = node.child(const_1407);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         try
         {
            node = config.child(const_1400)[0];
            if(node != null)
            {
               list = node.child(const_1403);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         this.error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1616);
         if(!disposed)
         {
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_792)
         {
            this.var_792.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_792)
         {
            ...
            this.var_792.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_792)
                  {
                     this.var_792.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_533 - this.var_364.length,this.var_533,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:int = !!this.var_208 ? 0 : this.var_1482 & 0;
         if(_loc3_ == Core.const_514)
         {
            this.var_260[param2].push(new UpdateDelegate(param1,this,param2));
         }
         else
         {
            this.var_260[param2].push(param1);
         }
      }
      ...
      override public function removeUpdateReceiver(param1:IUpdateReceiver) : void
      {
         ...
         var _loc3_:* = null;
         var _loc6_:* = null;
         var _loc4_:int = !!this.var_208 ? 0 : this.var_1482 & 0;
         var _loc5_:int = 0;
         while(_loc5_ < CoreComponentContext.const_105)
         {
            _loc3_ = this.var_260[_loc5_] as Array;
            if(_loc4_ == Core.const_514)
            {
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         this.var_365(_loc2_,_loc2_ - this._lastUpdateTimeMs);
      ...
      private function simpleFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         {
            this.var_647[priority] = 0;
            receivers = this.var_260[priority];
            index = 0;
            length = receivers.length;
            while(index != length)
            {
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         var maxTimePerFrame:uint = 1000 / var_400.stage.frameRate;
         priority = 0;
         while(priority < CoreComponentContext.const_105)
         {
            t = getTimer();
            ...
            {
               if(this.var_647[priority] < priority)
               {
                  ++this.var_647[priority];
                  skip = true;
               }
            }
            if(!skip)
         ...
         while(priority < CoreComponentContext.const_105)
         {
            ...
            while(index != length)
            {
               ...
               else
               {
                  try
                  {
                     this.var_208.update(receiver,msSinceLastUpdate);
                  }
                  catch(e:Error)
                  {
                     error("Error in update receiver \"" + getQualifiedClassName(receiver) + "\": " + e.message,true,e.errorID,e);
