package com.sulake.core.window.services
{
   ...
   public class WindowMouseOperator implements IDisposable
   {
      ...
      protected var var_147:DisplayObject;
      ...
      public function begin(param1:IWindow, param2:uint = 0) : IWindow
      {
         ...
         {
            this.var_147.addEventListener(MouseEvent.MOUSE_DOWN,this.handler,false);
            this.var_147.addEventListener(MouseEvent.MOUSE_UP,this.handler,false);
            this.var_147.addEventListener(Event.ENTER_FRAME,this.handler);
            this._mouse.x = this.var_147.mouseX;
            this._mouse.y = this.var_147.mouseY;
      ...
      public function end(param1:IWindow) : IWindow
      {
         ...
         if(this._working)
         {
            ...
            {
               this.var_147.removeEventListener(MouseEvent.MOUSE_DOWN,this.handler,false);
               this.var_147.removeEventListener(MouseEvent.MOUSE_UP,this.handler,false);
               this.var_147.removeEventListener(Event.ENTER_FRAME,this.handler);
      ...
      protected function handler(param1:Event) : void
      {
         ...
         if(this._working)
         {
            if(param1.type == Event.ENTER_FRAME)
            {
               ...
               else if(this._mouse.x != this.var_147.mouseX || this._mouse.y != this.var_147.mouseY)
               {
                  this.operate(this.var_147.mouseX,this.var_147.mouseY);
                  this._mouse.x = this.var_147.mouseX;
                  this._mouse.y = this.var_147.mouseY;
               }
               return;
      ...
      public function operate(param1:int, param2:int) : void
      {
         ...
         this.getMousePositionRelativeTo(this._window,this._mouse,this.var_699);
         this._window.offset(this.var_699.x - this._offset.x,this.var_699.y - this._offset.y);
      }
