package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_830:IFigurePartSet;
      ...
      private var var_1966:BitmapData;
      
      private const const_2059:int = 13421772;
      
      private const const_2058:int = 16777215;
      
      private const const_361:Array = [2,6,0,4,3,1];
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 == null)
         {
            this.var_695 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_1964 = Math.max(this.var_1964,_loc6_.colorLayerIndex);
            }
         }
         this.var_454 = this.var_68.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_695 != null && !this.var_1965)
            {
               _loc3_ = this.var_695;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_830)
            {
               switch(this.var_830.clubLevel)
               {
                  case HabboClubLevelEnum.const_54:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_44:
                     _loc2_.style = HabboIconType.const_818;
                     break;
                  case HabboClubLevelEnum.const_47:
                     _loc2_.style = HabboIconType.const_842;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this.var_1423.color = this.const_2059;
         }
         else
         {
            this.var_1423.color = this.const_2058;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_454.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_454.isFigureReady(_loc1_))
         {
            this.var_454.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_361[_loc6_] + "_" + FigureData.const_139;
               _loc4_ = this.var_454.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
               while(!_loc7_ && _loc6_ < this.const_361.length)
               {
                  _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_361[_loc6_] + "_" + FigureData.const_139;
                  _loc4_ = this.var_454.getAssetByName(_loc3_) as BitmapDataAsset;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_670 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1966)
               {
                  ...
                  this.var_1966 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_670.width,this.var_670.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_670.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_670.y;
               if(this.var_1965 && _loc2_.colorLayerIndex > 0)
               {
