package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_885:int = 3;
      
      private var var_407:AssetLoaderStruct;
      
      private var var_884:Timer;
      ...
      private const const_2051:int = 100;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_407 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_884 = new Timer(this.const_2051,1);
         this.var_884.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_407)
         {
            this.var_407.addEventListener(AssetLoaderEvent.const_29,this.onConfigurationComplete);
            this.var_407.addEventListener(AssetLoaderEvent.const_48,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_407)
         {
            this.var_407.removeEventListener(AssetLoaderEvent.const_29,this.onConfigurationComplete);
            this.var_407.removeEventListener(AssetLoaderEvent.const_48,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_885);
            if(this.var_1347.indexOf("?") > 0)
            {
               _loc2_ = this.var_1347 + "&retry=" + this.var_885;
            }
            else
            {
               _loc2_ = this.var_1347 + "?retry=" + this.var_885;
            ...
            this.var_407 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2157,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1088[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1624)
         {
            this.var_1087.push([param1,param2]);
            return;
         }
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_886)
         {
            _loc7_ = true;
            _loc4_ = this.var_886[_loc3_];
         ...
         {
            delete this.var_886[_loc3_];
         }
         while(_loc6_ < this.var_711.length)
         {
            _loc5_ = this.var_711[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_711.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_884.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_710.length > 0 && this.var_711.length < this.const_1487)
         {
            _loc1_ = this.var_710[0];
            _loc1_.startDownloading();
            this.var_711.push(this.var_710.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_710.indexOf(param1) == -1 && this.var_711.indexOf(param1) == -1)
         {
