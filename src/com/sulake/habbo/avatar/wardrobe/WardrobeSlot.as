package com.sulake.habbo.avatar.wardrobe
{
   ...
   public class WardrobeSlot implements IOutfit, IAvatarImageListener
   {
      ...
      private function createView() : void
      {
         ...
         this.var_388 = this._view.findChildByName("image") as IBitmapWrapperWindow;
      }
      ...
      public function updateView() : void
      {
         ...
         if(this.var_683 && this.var_1573)
         {
            _loc5_ = this._controller.avatarRenderManager.createAvatarImage(this.figure,AvatarScaleType.const_93,this.var_1048,this);
            if(_loc5_)
            {
               _loc5_.setDirection(AvatarSetType.const_37,parseInt(FigureData.const_869));
         ...
         if(this.var_388)
         {
            if(this.var_388.bitmap)
            {
               this.var_388.bitmap.dispose();
            }
            this.var_388.bitmap = new BitmapData(this.var_388.width,this.var_388.height,true,0);
            _loc7_ = (this.var_388.width - _loc1_.width) / 2;
            _loc8_ = (this.var_388.height - _loc1_.height) / 2;
            this.var_388.bitmap.draw(_loc1_,new Matrix(1,0,0,1,_loc7_,_loc8_));
         }
         if(_loc2_)
         {
            _loc1_.dispose();
      ...
      private function eventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            case "set_button":
               this.var_683 = this._controller.figureData.getFigureString();
               this.var_1048 = this._controller.gender;
               this._controller.handler.saveWardrobeOutfit(this.var_2035,this);
               this.updateView();
               break;
            case "get_button":
               if(this.var_683)
               {
                  this._controller.loadAvatarInEditor(this.var_683,this.var_1048,this._controller.clubMemberLevel);
               }
         }
      }
