package com.sulake.habbo.communication.demo
{
   ...
   public class HabboLoginDemoView extends EventDispatcher
   {
      ...
      public static const const_605:String = "InitConnection";
       
      
      private var var_161:HabboCommunicationDemo;
      
      private var _window:IWindowContainer;
      
      private var var_133:SSOTicket;
      ...
      {
         this.var_161.windowManager.removeWindow("habbo_login_window");
      }
      ...
      {
         var _loc1_:XmlAsset = (this.var_161 as Component).assets.getAssetByName("login_window") as XmlAsset;
         if(_loc1_ == null)
         {
            Logger.log("Could  not find embedded Window XML for Login Window, login_window");
            return;
         }
         this._window = this.var_161.windowManager.createWindow("habbo_login_window","Login",HabboWindowType.const_139,HabboWindowStyle.const_45,0 | 0,new Rectangle(10,10,320,240),null) as IFrameWindow;
         var _loc2_:XML = _loc1_.content as XML;
         var _loc3_:IWindowContainer = this.var_161.windowManager.buildFromXML(_loc2_) as IWindowContainer;
         ...
         var _loc7_:SharedObject = SharedObject.getLocal(this.const_1801,"/");
         if(_loc5_ != null)
         {
         ...
         if(_loc8_)
         {
            if(this.var_161.habboConfiguration)
            {
               if(this.var_161.habboConfiguration.getKey("use.sso","com.sulake.habbo.inventory.marketplace") == "InfoStandSongDiskView")
               {
                  _loc8_.select();
               }
            }
      ...
      public function populateUserList(param1:Map) : void
      {
         ...
         var _loc3_:XmlAsset = this.var_161.assets.getAssetByName("login_user_list_item") as XmlAsset;
         var _loc4_:IWindow = this.var_161.windowManager.buildFromXML(_loc3_.content as XML);
      ...
      private function windowEventProcessor(param1:WindowEvent = null, param2:IWindow = null) : void
      {
         ...
         if(_loc3_ != null)
         {
            ...
            _loc6_ = SharedObject.getLocal(this.const_1801,"/");
         ...
         {
            dispatchEvent(new Event(const_605));
         }
      }
      ...
      private function listEventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_161.sendTryLogin(this.name,this.password,param2.id);
      }
      ...
      private function initSSOTicket(param1:Boolean) : void
      {
         ...
         {
            this.var_133.removeEventListener(SSOTicket.const_379,this.onParsedTicketSuccess);
            this.var_133.removeEventListener(SSOTicket.const_337,this.onParsedTicketFailure);
            this.var_133 = null;
         }
         var _loc2_:String = "";
         if(this.var_161.habboConfiguration != null)
         {
            _loc2_ = this.var_161.habboConfiguration.getKey("url.prefix",_loc2_);
            _loc2_ = _loc2_.replace("http://","");
            _loc2_ = _loc2_.replace("https://","");
         }
         var _loc3_:IHabboWebLogin = this.var_161.habboCommunication.habboWebLogin(this.name,this.password);
         this.var_133 = new SSOTicket(this.var_161.assets,_loc3_,_loc2_,param1);
         this.var_133.addEventListener(SSOTicket.const_379,this.onParsedTicketSuccess);
         this.var_133.addEventListener(SSOTicket.const_337,this.onParsedTicketFailure);
      }
      
      private function onParsedTicketSuccess(param1:Event) : void
      {
         Logger.log("Got ticket: " + this.var_133.ticket);
         Logger.log("Has Facebook: " + this.var_133.isFacebookUser);
         if(this.var_133.isFacebookUser)
         {
            this.var_161.habboConfiguration.setKey("facebook.user","1");
         }
         this.var_161.flashClientUrl = this.var_133.flashClientUrl;
         this.var_161.ssoTicket = this.var_133.ticket;
         this.var_133.removeEventListener(SSOTicket.const_379,this.onParsedTicketSuccess);
         this.var_133.removeEventListener(SSOTicket.const_337,this.onParsedTicketFailure);
         this.var_133 = null;
         dispatchEvent(new Event(const_605));
