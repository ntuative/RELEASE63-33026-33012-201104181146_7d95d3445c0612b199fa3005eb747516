package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class BotCatalogWidget extends CatalogWidget implements ICatalogWidget, IItemGrid, IAvatarImageListener
   {
      ...
      private var var_1526:Map;
      
      private var var_829:Offer;
      
      protected var var_566:IItemGridWindow;
      
      protected var var_1713:XML;
      
      private var var_1954:Map;
      
      private var var_394:IGridItem;
      
      public function BotCatalogWidget(param1:IWindowContainer)
      {
         super(param1);
         this.var_566 = _window.findChildByName("botGrid") as IItemGridWindow;
         this.var_1954 = new Map();
      }
      ...
      {
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(this.var_829,page.pageId);
      }
      
      private function onBotDefinitions(param1:RentableBotDefinitionWidgetEvent) : void
      {
         this.var_1526 = param1.botFigureData;
      ...
      {
         var _loc6_:* = null;
         var _loc2_:IWindowContainer = page.viewer.catalog.windowManager.buildFromXML(this.var_1713) as IWindowContainer;
         this.var_566.addGridItem(_loc2_);
         param1.productContainer.view = _loc2_;
         var _loc3_:String = this.var_1526.getValue(param1.productContainer.firstProduct.extraParam.toLowerCase());
         ...
         var _loc5_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createAvatarImage(_loc3_,AvatarScaleType.const_93,null,this);
         if(_loc5_)
         {
            _loc5_.setDirection(AvatarSetType.const_37,4);
            _loc6_ = _loc5_.getImage(AvatarSetType.const_37,true);
            _loc4_.bitmap.copyPixels(_loc6_,_loc6_.rect,new Point((_loc4_.width - _loc6_.width) / 2,_loc4_.height - _loc6_.height));
            _loc4_.invalidate();
            this.var_1954.add(_loc3_,param1.offerId);
            _loc5_.dispose();
         }
      }
      ...
      {
         var _loc1_:String = this.var_1526.getValue(this.var_829.productContainer.firstProduct.extraParam.toLowerCase());
         var _loc2_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createAvatarImage(_loc1_,AvatarScaleType.const_53);
         if(_loc2_)
         {
            _loc2_.setDirection(AvatarSetType.const_37,4);
            this.setPreviewImage(_loc2_.getImage(AvatarSetType.const_37,true));
            _loc2_.dispose();
         }
      }
      ...
      private function setProductData() : void
      {
         ...
         if(_loc2_ != null)
         {
            ...
            _loc1_.registerParameter(_loc5_,"credits",String(this.var_829.priceInCredits));
            _loc1_.registerParameter(_loc5_,"pixels",String(this.var_829.priceInActivityPoints));
            _loc2_.text = _loc1_.getKey(_loc5_);
         }
         var _loc3_:IProductData = page.viewer.catalog.getProductData(this.var_829.localizationId);
      ...
      public function avatarImageReady(param1:String) : void
      {
         ...
         var _loc7_:* = null;
         var _loc8_:* = null;
         var _loc2_:int = this.var_1954.getValue(param1);
         if(true)
         {
            return;
         }
         for each(_loc3_ in page.offers)
         {
            ...
            {
               _loc4_ = this.var_1526.getValue(_loc3_.productContainer.firstProduct.extraParam.toLowerCase());
               _loc5_ = _loc3_.productContainer.view;
               _loc6_ = _loc5_.findChildByName("image") as IBitmapWrapperWindow;
               _loc7_ = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createAvatarImage(_loc4_,AvatarScaleType.const_93,null);
               if(_loc7_)
               {
                  _loc7_.setDirection(AvatarSetType.const_37,4);
                  _loc8_ = _loc7_.getImage(AvatarSetType.const_37,true);
                  _loc6_.bitmap.copyPixels(_loc8_,_loc8_.rect,new Point((_loc6_.width - _loc8_.width) / 2,_loc6_.height - _loc8_.height));
                  _loc6_.invalidate();
                  _loc7_.dispose();
                  if(this.var_829.offerId == _loc2_)
                  {
                     this.updatePreviewImage();
                  }
               }
