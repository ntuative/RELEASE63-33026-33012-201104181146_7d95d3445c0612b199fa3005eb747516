package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      private var var_125:GuestRoomData;
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2575 = new RoomThumbnailRenderer(param1);
         this.var_1178 = new TagRenderer(param1,hideInstantly);
      }
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_191 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_191.visible = true;
         Util.hideChildren(this.var_191);
         this.refreshRoomName(this.var_191,this.var_125);
         if(this.var_125.ownerName != "" && this.var_125.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_191,!!this.var_125.event ? this.getEventCtg(this.var_125.categoryId) : this.getRoomCtg(this.var_125.categoryId));
         this.refreshRoomDesc(this.var_191,this.var_125);
         this.refreshExtraCont();
         this.refreshInfo(this.var_191,"trading_allowed",this.var_125.allowTrading);
         this.refreshInfo(this.var_191,"doormode_doorbell",this.var_125.doorMode == 1);
         this.refreshInfo(this.var_191,"doormode_password",this.var_125.doorMode == 2);
         Util.moveChildrenToColumn(this.var_191,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_191.height = Util.getLowestPoint(this.var_191);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_191,this.var_125.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_191.findChildByName("roomowner"));
         if(this.var_125.ownerName != "" && this.var_125.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_191.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_1178.refreshTags(_loc1_,this.var_125.tags);
         this.refreshTextWithCaption("rating",_loc1_,"" + this.var_125.score);
         if(this.var_125.event)
         {
            this.refreshTextWithCaption("startedat",_loc1_,"" + this.var_125.eventCreationTime);
         }
         this.var_2575.refreshThumbnail(_loc1_,this.var_125.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_254,Tabs.const_256,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
