package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private function prepareWindow() : void
      {
         ...
         this._window = IFrameWindow(this.var_79.getXmlWindow("ude_help"));
         this.var_1948 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_1240 = this.var_1948.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_1240)
         {
            this.var_1240.procedure = this.onButtonClick;
         }
         this.var_1239 = this.var_1948.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_1239)
         {
            this.var_1239.procedure = this.onButtonClick;
         }
      ...
      {
         if(this.var_269 >= this.var_62.length - 1)
         {
            this.var_1239.disable();
         }
         else
         {
            this.var_1239.enable();
         }
         if(this.var_269 <= 0)
         {
      ...
      {
         ++this.var_269;
         this.var_269 = Math.min(this.var_269,this.var_62.length - 1);
         this.openPage();
      }
      ...
      {
         --this.var_269;
         this.var_269 = Math.max(this.var_269,0);
         this.openPage();
      }
      ...
      private function initLocalizations() : void
      {
         ...
         this.var_62 = [];
         var _loc1_:int = 1;
         while(this.var_79.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title"))
         {
            _loc2_ = this.var_79.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title");
            _loc3_ = this.var_79.localization.getLocalization("wiredfurni.help." + _loc1_ + ".img");
            _loc4_ = this.var_79.localization.getLocalization("wiredfurni.help." + _loc1_ + ".text");
            this.var_62.push([_loc2_,_loc3_,_loc4_]);
            _loc1_++;
         }
      }
      ...
      private function localize() : void
      {
         ...
         if(!this.var_62 || this.var_62.length == 0)
         {
            return;
         }
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this.var_62[this.var_269][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this.var_62[this.var_269][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this.var_62[this.var_269][1]).value;
      ...
      {
         if(!param1 || !this.var_79 || !this.var_79.assets)
         {
            return;
         }
         var _loc2_:BitmapDataAsset = this.var_79.assets.getAssetByName(param1) as BitmapDataAsset;
         if(_loc2_ == null)
         {
      ...
      private function retrievePreviewAsset(param1:String) : void
      {
         ...
         var _loc2_:String = this.var_79.configuration.getKey("image.library.catalogue.url");
         var _loc3_:* = _loc2_ + param1 + ".gif";
         Logger.log("[ProductViewCatalogWidget] Retrieve Product Preview Asset: " + _loc3_);
         var _loc4_:URLRequest = new URLRequest(_loc3_);
         var _loc5_:AssetLoaderStruct = this.var_79.assets.loadAssetFromFile(param1,_loc4_,"image/gif");
         if(!_loc5_)
         {
            return;
         }
